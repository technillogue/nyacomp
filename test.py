...
print(f"loading with gpu decompression took {time.time() - comp_start:.4f}")
torch.save(original_tensor, "/tmp/ones.pth")
unpickle_start = time.time()
unpickled = torch.load("/tmp/ones.pth").cuda()
print(f"torch.load took {time.time() - unpickle_start:.4f}")
