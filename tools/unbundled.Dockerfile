...
RUN --mount=type=cache,target=/var/cache/apt apt-get update && apt-get install -yy libnvidia-compute-515-server python3.11 patchelf zip unzip curl build-essential gcc libpython3.11-dev 
...
RUN --mount=type=cache,target=/root/.cache/pip pip install pybind11 --extra-index-url https://download.pytorch.org/whl/cu118 torch==2.0.1+cu118
